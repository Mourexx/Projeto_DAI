...
class Viagem(Base):
    """
    Representa uma viagem concreta de um autocarro numa linha.
    Uma viagem tem um início, um fim, e regista a contagem de passageiros.
    """
    ...
    # Contagem de passageiros (atualizada em tempo real)
    total_entradas = Column(Integer, default=0)
    total_saidas = Column(Integer, default=0)
    ocupacao_atual = Column(Integer, default=0)
    capacidade_maxima = Column(Integer, default=60)        # Capacidade do autocarro

    # Localização atual (atualizada em tempo real)
